...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0x00000000fc17f0 | 0x00000000fc17f0 | 0x00000000fc1830 |
| `newData` 地址 | 尚未申请，不填写 | 0x0000000010a1830 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不可以。`delete[]` 执行后，旧数组所占用的内存已经被释放归还给操作系统，虽然 Memory View 可能仍显示残留字节，但该内存区域已不再属于程序的合法访问范围，继续访问属于未定义行为，可能导致程序崩溃或数据损坏。
...
> 扩容时必须先复制再释放旧数组，因为如果先释放旧数组，旧数组中的数据就丢失了，无法复制到新数组中。扩容后不能直接把 size 设成 capacity，因为 size 表示有效元素个数，capacity 表示数组总容量，两者含义不同，新分配的空间尚未存入有效元素。
