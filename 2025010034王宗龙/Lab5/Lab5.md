...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0x0000000000fe1900 | 0x0000000000fe1900 | 0x0000000000fe1940 |
| `newData` 地址 | 尚未申请，不填写 | 0x0000000000fe1940 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不可以继续在程序中访问旧数组。因为 `delete[]` 已经释放了旧数组的内存，该内存区域可能已被系统回收或分配给其他用途，继续访问会导致未定义行为，可能引发程序崩溃或数据损坏。
...
> 扩容时必须先复制再释放旧数组，因为如果先释放旧数组，旧数组中的数据就会丢失，无法复制到新数组中。扩容后不能直接把 `size` 设成 `capacity`，因为 `size` 表示有效元素的个数，而 `capacity` 表示数组的总容量，扩容只是增加了可用空间，并没有增加有效元素。
