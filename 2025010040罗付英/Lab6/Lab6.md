...
| :--- |:-------------------|:-------------------|:-----------------------|
| `p` 的地址 | 0x0000000000c71700 | 0x0000000000c71700 | 0x0000000000ad1740     |
| `p->data` | 18                 | 18                 | 18                     |
| `p->next` 的地址 | 0x00000000001c1708 | 0x00000000001c1708                   | 0x00000000001c1708     |
| `prev->next` 的地址（图 C 填 `list->head->next`） | NULL               | 0x0000000000b01840 | 0x0000000000771700     |
| `list->size` | 6                  | 6                  | 6                      |
...
> 在此填写你的回答（不少于 20 字）。
> p结点的16个字节没有发生变化；prev结点中next成员的8字节发生改变，data成员8字节保持不变。

2. 图 C 里 `p` 的 `next` 还指着 25 所在的结点，为什么这不会导致 25 被重复释放？如果把 `listRemove` 里的 `free(p);` 删掉，程序输出会不会变？会少掉什么？

> 在此填写你的回答（不少于 30 字）。
> p只是局部指针变量，free只释放p指向的结点内存，不会释放p‑>next指向结点，因此不会重复释放。删掉 free(p); 程序输出不变，但会发生内存泄漏，被删除的结点内存得不到回收。
...
| `unlinkAfter` 里"后面没有结点"的判断 | `p == NULL` | p->next == head       |
| 表尾追加复杂度 | `O(n)` | O(n)                  |
| 从最后一个结点再走一步到哪里 | 无处可走 | 回到头节点head        |
...
> 在此填写你的回答（不少于 60 字）。
> 链表在最前面插入数据很快，不用挪动别的数据，时间O(1)；但是往最后面加数据就慢，得从头跑到尾找到末尾，时间O(n)。
向量刚好反过来，往末尾加数据很快，均摊O(1)；但如果在最开头插入，所有老数据都要往后挪，速度就慢，时间O(n)。
链表适合经常在开头、中间增删数据，很少按下标随机读取数据的情况。
