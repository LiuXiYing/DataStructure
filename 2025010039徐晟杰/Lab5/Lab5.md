...
> 不可以。旧数组已被 `delete[]` 释放，这块内存不再属于本对象，继续访问属于使用已释放的内存，行为未定义，可能读到垃圾数据甚至导致程序崩溃。调试器中残留的字节只是显示结果，并不代表该空间仍然有效。
...
> 扩容时旧数组的数据仍然有效，必须先把有效元素复制到新数组，再释放旧数组；如果先释放，旧数据就丢失，也无法再读取。扩容只增加可用空间，不增加有效元素个数，因此 `size` 必须保持原值，不能直接设成 `capacity`，否则会把未写入的备用位置当成有效元素。
