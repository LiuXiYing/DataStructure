...
>图 A 到图 B 之间，p 结点的 16 个字节没有任何变化，因为 Step Over 只执行了 prev->next = p; 这一句，它修改的是 prev 的指针域，不是 p 的内容。prev 结点的 16 个字节中，只有第 8 到 15 字节（即 next 指针）变了，从原来的 0x00b31650（指向 42 所在结点）变成了 0x00b31710（指向 p），前 4 字节的 data（-1）和中间 4 字节的对齐填充都没有变化。

2. 图 C 里 `p` 的 `next` 还指着 25 所在的结点，为什么这不会导致 25 被重复释放？如果把 `listRemove` 里的 `free(p);` 删掉，程序输出会不会变？会少掉什么？

> p->next 指向 25 所在结点不会导致 25 被重复释放，因为 free 只释放 p 自己，不会沿着 next 链继续释放。unlinkAfter 已经把 p 从链表中断开了，链表后续操作不会再经过 p，所以 p->next 虽然还指着 25，但没有任何代码会通过 p 去访问或释放 25。如果把 free(p); 删掉，程序输出不会变化，但会造成内存泄漏——被摘下的结点占用的堆内存永远不会被回收。最后 listDestroy 打印的释放结点数会少一个（变成 6 而不是 7），说明有一个结点泄漏了。
...
> listPushBack 是 O(n)，因为每次都要从头遍历到末尾找到最后一个结点才能追加；IntVector::pushBack 是均摊 O(1)，因为数组有预留空间，直接在末尾写入即可。但 listInsert(list, 0, v) 是 O(1)，只需改两个指针；IntVector 在下标 0 处插入是 O(n)，需要把所有元素往后挪一位。频繁在中间或头部插入删除时选链表，需要随机访问或追求缓存效率时选向量。
