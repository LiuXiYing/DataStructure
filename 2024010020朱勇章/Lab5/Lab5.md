...
如果希望同时观察变量和内存，可在 Variables/Watches 工具栏使用“在编辑器中打开内存视图”的按钮（官方文档中的 **Open memory view in the editor**），把生成的内存标签拖到编辑区一侧形成分栏，再将 Debug 窗口切回 Variables。变量用于帮助观察和填写记录，截图只需看清内存地址及关键十六进制数据。
...
暂停在 `int newCapacity = capacity * 2;`，通过 `data` 打开 Memory View。截图能看清旧数组地址，以及从该地址开始的四个整数 `10、20、30、40` 的十六进制内容即可。

此时应为 `size == 4`、`capacity == 4`，供填写观察记录时参考，无需在截图中展示这些变量或当前代码行。
...
在 `expand()` 的 `delete[] data;` 行设置第二个断点，继续运行到此处，先不要执行这一行。通过局部指针 **`newData`** 打开 Memory View。截图能看清与图 A 不同的新地址，以及新地址处四个整数 `10、20、30、40` 的十六进制内容即可。

此时对象仍为 `size == 4`、`capacity == 4`，局部变量 `newCapacity == 8`。填写记录时分别查看旧 `data` 和新 `newData` 的地址；截图无需同时显示两个指针或旧内存窗口。新数组尚未使用的位置不要求为零。
...
在 Variables 中展开对象 **`v`**，从它的 `data` 成员重新打开 Memory View。截图能看清图 B 中的新地址，以及在原有 `10、20、30、40` 后追加的 `50` 的十六进制内容即可。

此时 `v` 的 `size == 5`、`capacity == 8`，供填写观察记录时参考。截图无需保留展开的 `v` 或当前代码行。
...
在 Variables/Watches 的表达式观察区域查看 `sizeof(int)`，把结果填入 3.6.4 的观察记录，截图无需保留该表达式及其值。常见环境中它为 `4`；在四字节 `int`、小端环境里，下面的字节对应这些整数：
...
图 A、B 需要看清前四个整数，图 C 需要看清前五个整数；不要把右侧 ASCII 字符栏当作十进制整数。可把每行字节数调整为 `16`，便于观察。字节序、大小写和显示分组以实际环境为准，不要求所有机器显示成完全相同的布局。
...
**`data` 行的三格和 `newData` 行的图 B 一格必须填写实际地址，不能留空或保留“填写”“（填写）”等占位文字。** 明确标注“不填写”的两格除外。

| 记录项目 | 图 A：扩容前 | 图 B：复制完成 | 图 C：追加完成 |
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0x1717f0 | 0x1717f0 | 0x174e00 |
| `newData` 地址 | 尚未申请，不填写 | 0x174e00 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不可以。`delete[] data;` 已经把这块动态数组归还给堆管理器，旧地址处的字节只是未被立即覆写的残留内容，不代表这块内存仍然属于本程序。此时再通过旧指针读取或写入就是悬空指针访问，属于未定义行为，可能读到垃圾值或破坏其他数据，所以调试器里看到的残留字节不能当作可用数据。

#### 3.6.5 截图检查标准

| 检查项目 | 要求 |
| :--- | :--- |
| 真实界面 | 来自本人电脑的真实 Memory View，使用系统截图功能；不使用手机拍屏、他人图片、示意图或拼接修改的结果 |
| 必要内容 | 每张图能看清内存地址及关键十六进制数据即可；无需展示代码行、变量区、计数、展开的对象或 `sizeof(int)` |
| 三个状态 | 图 A 为四个整数 `10、20、30、40`；图 B 为新地址的这四个整数；图 C 为新地址在这四个整数后追加 `50`，均以十六进制显示 |
| 地址关系 | 以实际内存数据所在行的地址为准，图 B 与图 A 不同，图 C 与图 B 相同；允许大小写和前导零差异 |
| 文件 | 三张真实 PNG，分别保存为指定文件名；每张不超过 5 MB，三张合计不超过 12 MB，最长边不超过 4096 像素，文字和字节仍须清晰 |
| 不作为错误的差异 | CLion 版本、操作系统、主题、字节高亮、地址具体数值、备用位置是否为零和窗口排列方式；地址表达式报错但下方仍清楚显示所需地址和字节时也可通过 |
...
> 如果先 `delete[]` 释放旧数组，原来的四个有效元素就无处可取，新数组会是未初始化的垃圾值；而且 `newData` 刚指向新数组、`data` 仍是旧数组唯一指针，一旦先删就丢失旧数据，所以必须先把旧元素逐个复制进新数组，再释放旧数组、最后更新 `data` 和 `capacity`。扩容只是申请了更大的空间，实际有效元素个数并没有变，`size` 仍等于扩容前的值；若直接把 `size` 设成 `capacity`，会把尚未写入数据的备用位置误当成有效元素，后续读取、打印和删除都会越界读到垃圾值，因此 `size` 保持不变。
