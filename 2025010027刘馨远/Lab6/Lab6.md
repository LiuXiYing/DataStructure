...
| `p->data` | 25 | 25 | 18 |
| `p->next`  | 0xb01740 | 0xb01740 | 0xb01780|
...
> 在此填写你的回答（不少于 60 字）。
链表慢在表尾追加时，找尾必须遍历O(n)；插入快在插入节点本身只改变指针，改指针O(1)。
向量快：尾部直接写，均摊O(1)；头部插入慢：要移动所有元素o(n)。
频繁插入选链表，频繁随机访问或尾部操作选向量。
---
