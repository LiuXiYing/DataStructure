...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0x000001e6c4a0f5e0 | 0x000001e6c4a0f5e0 | 0x000001e6c4a0f620 |
| `newData` 地址 | 尚未申请，不填写 | 0x000001e6c4a0f620 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不能继续在程序中访问旧数组。因为 `delete[]` 操作已经释放了旧数组所占用的内存空间，这块内存可能已被系统回收或分配给其他程序使用。虽然调试器可能仍显示残留字节，但这些数据不再属于当前程序，访问已释放的内存会导致未定义行为，可能引发程序崩溃或数据损坏。
...
> 扩容时要先复制再释放旧数组，是因为如果先释放旧数组，旧数组中的数据就会丢失，无法复制到新数组中。扩容后不能直接把 `size` 设成 `capacity`，因为 `size` 表示有效元素的个数，而 `capacity` 表示数组的总容量，扩容只是增加了数组的容量，并没有增加有效元素的个数。
