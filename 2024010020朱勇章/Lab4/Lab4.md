...
| 返回 `int` 的 `1` / `0` | 返回 `bool` 类型的 `true` / `false` |

### 4.13 填空：一句话总结

Lab3 的 `main` 里可以直接写 `list.size = 4;`。用一句话说明：为什么把 `size` 设为 `private` 之后，`insert`、`remove` 里的边界检查才真正"管用"？

> 把 `size` 设为 `private` 后，类外不能再直接读写它，任何对元素个数的改动都必须经过 insert / remove / get 这些成员函数。这些函数在入口处统一做边界检查（下标是否越界、是否满、是否空表），非法操作会被挡在函数内；如果 `size` 是 public，外部代码可以绕过检查直接 `v.size = -1` 或随便增减，对象的内部不变量（data 里实际元素个数与 size 一致）就可能被破坏，而 private 从语法层面杜绝了这种绕过。
