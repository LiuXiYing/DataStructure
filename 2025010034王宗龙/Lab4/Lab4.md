...
| C（Lab3 / 课堂） | C++（本次作业） |
| :--- | :--- |
| `SeqList list; list.size = 0;` | `IntVector v;`（构造函数自动初始化） |
| `insertAt(&list, 2, 25)` | `v.insert(2, 25)` |
...
> 因为 `size` 是 `private` 的，类外代码（如 `main`）无法直接修改 `size`，只能通过 `insert`、`remove` 等成员函数来修改，而这些成员函数内部都有边界检查，从而保证了 `size` 始终是合法的。
