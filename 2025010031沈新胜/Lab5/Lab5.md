...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0xf81980 | 0xf81980 | 0xf819c0 |
| `newData` 地址 | 尚未申请，不填写 | 0xf819c0 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不可以。`delete[] data` 执行后，旧数组的内存已交还给运行时内存分配器，随时可能被重新分配或处于未定义状态；调试器中仍能看到字节只是残留内容，并不代表该内存仍归程序使用。继续访问释放后的内存属于未定义行为，可能导致数据错乱或程序崩溃，因此不能在释放后访问旧数组。
...
> 扩容时必须先复制再释放旧数组，是因为新数组的全部有效元素都要从旧数组读取，旧数组是唯一的数据来源；若先释放旧数组，就失去了旧元素内容，无法安全完成复制。扩容只是预留更多空间，并没有向容器放入任何新元素，有效元素个数仍是原来的 `size`；若直接把 `size` 设成 `capacity`，会把尚未写入数据的备用位置误当成有效元素，破坏 `size` 与“有效元素个数”的对应关系。
