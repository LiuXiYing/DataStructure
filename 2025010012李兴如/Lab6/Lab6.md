...
| :--- |:-----------------|:---------------|:-----------------------|
| `p` 的地址 | 0xb01740         | 0xb01740       | 0xb01740               |
| `p->data` | 25               | 25             | -1                     |
| `p->next` 的地址 | 0xb01780         | 0xb01780       | 0xb01840               |
| `prev->next` 的地址（图 C 填 `list->head->next`） | 0xb01780         | 0xb01740       | 0xb01780               |
| `list->size` | 6                | 7              | 6                      |
...
> 没有发生任何变化，Prev后面8节存储next指针部分发生改变，由原来指向42节点的地址修改为p节点的位置。

2. 图 C 里 `p` 的 `next` 还指着 25 所在的结点，为什么这不会导致 25 被重复释放？如果把 `listRemove` 里的 `free(p);` 删掉，程序输出会不会变？会少掉什么？

> 因为p从链表中摘出，只是残留的一部分，free只会执行一次；删掉free不会影响结果，但会导致内存泄露，内存不会被回收。
...
- [x] A. `prev->next = p; p->next = prev->next;`
...
- [x] C. 只写一句 `free(list->head);`，后面的结点会跟着一起释放
...
>listpushback需要遍历到最后以为，所以是o(n),IntVector::pushBack依靠分配空间，listInsert(list, 0, v)在头哨兵后面插入，链表快在头部插入，慢在尾部追加；而向量快在随机访问和尾部追加，慢在头部插入，在需要大量在表头或中间插入时选择链表，其余选向量。
