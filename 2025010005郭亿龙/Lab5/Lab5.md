...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | （请填写调试中看到的实际十六进制地址） | （请填写调试中看到的实际十六进制地址） | （请填写调试中看到的实际十六进制地址） |
| `newData` 地址 | 尚未申请，不填写 | （请填写调试中看到的实际十六进制地址） | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
不可以。`delete[]` 之后旧数组已被释放，它指向的地址不再有效，继续访问属于未定义行为，可能会读到乱码甚至导致程序崩溃。
...
扩容时先复制再释放，是因为旧数组中还保留着原有数据，必须先把它们完整复制到新数组，才可以安全释放旧数组并让 `data` 指向新数组。扩容只负责准备更大的空间，`size` 记录的是有效元素个数，直接设成 `capacity` 会凭空多出大量无效元素，破坏 `0 <= size <= capacity` 的约定。
