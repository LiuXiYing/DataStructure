...
| :--- |:------------------|:------------------|:---------------------|
| 当前 `data` 地址（图 C 为 `v.data`） | 0x00000000000ed190 | 0x00000000000ed190 | 0x00000000000ed940   |
| `newData` 地址 | 尚未申请，不填写  | 0x00000000000ff19 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4                 | 4                 | 5                    |
| 当前对象的 `capacity` | 4                 | 8                 | 8s                   |
...
不可以。执行 delete [] 释放数组后，这块内存已经归还给操作系统。调试器看到残留数据仅仅是内存内容还没有被覆盖，此时访问旧地址属于野指针访问，属于未定义行为，会引发程序崩溃或者内存错误。> 在此填写你的回答（不少于 20 字）。
...
| :--- | :---: | :---: | :---: |
| `IntVector v;` 创建后 | 0（填写） | 4（填写） | 0（填写） |
| `v` 追加 `40` 后 | 4（填写） | 4（填写） | 0（填写） |
| 满表 `v.insert(5, 99)` 失败后 | 4（填写） | 4（填写） | 0（填写） |
| `v` 追加 `50` 后 | 5（填写） | 8（填写） | 4（填写） |
| 满表 `v.insert(4, 28)` 后 | 9（填写） | 16（填写） | 8（填写） |
| 六次删除完成、`v` 删除全部元素后 | 0（填写） | 16（填写） | 0（填写） |

### 4.13 填空：解释扩容过程

用两三句话说明：为什么扩容时要先复制再释放旧数组？为什么扩容后不能直接把 `size` 设成 `capacity`？

扩容时要先申请新空间、复制旧数组数据，**之后再释放旧数组**，如果先释放旧数组，原有数据直接丢失无法复制。
size 代表**有效元素数量**，capacity 是总内存容量，扩容只是开辟更大内存，并没有新增有效元素，所以不能直接把 size 设为 capacity。> 在此填写你的回答（不少于 30 字）。
