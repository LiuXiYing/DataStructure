...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0x0000000000ec1920 | 0x0000000000ec1920 | 0x0000000000ec1960 |
| `newData` 地址 | 尚未申请，不填写 | 0x0000000000ec1960 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不能。`delete[]` 之后旧数组所在的堆内存已经归还分配器，`data` 若仍指向它就是一个悬空指针，后续的 `new` 可能把这块内存分给别人；继续通过旧指针读写属于未定义行为，可能读到已改动或无关的数据、导致程序崩溃甚至损坏其他对象。调试器显示残留字节只是内存未被立即改写，并不表示这块内存仍然属于我们。
...
> 扩容时必须先申请新数组并把旧元素复制过去，再释放旧数组，因为释放旧数组后旧地址立刻失效，先释放就拿不到已有元素的数据了。扩容只是扩大存储空间并转移数据，并不会往表里添加新元素，`size` 表示真实的有效元素个数，只有在 insert 真正写入元素时才加一；若直接把 `size` 设为 `capacity`，会把空白位置当成有效元素，print、find 等就会读到未定义的垃圾数据。
