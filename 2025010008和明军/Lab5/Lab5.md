...
2. 先在 `expand()` 中的 `int newCapacity = capacity * 2;` 行左侧设置断点。
3. 程序暂停后，在 Debug 窗口选中 `expand()` 栈帧，在 **Variables** 中展开 `this`，显示 `data`、`size` 和 `capacity`。
4. 右键 `data`，选择 **Show in Memory View**。
5. 用 **Step Over** 越过语句，避免进入 `new[]` 等库函数内部。
...
在 `main` 中，紧接 `v.pushBack(50);` 的 `std::cout << "追加 50 后: size = " ...` 行设置第三个断点，继续运行到此处。在 Variables 中展开对象 **`v`**，从它的 `data` 成员重新打开 Memory View。截图能看清图 B 中的新地址，以及在原有 `10、20、30、40` 后追加的 `50` 的十六进制内容即可。
...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0x0000000000e717f0 | 0x0000000000e717f0 | 0x0000000000fa1830 |
| `newData` 地址 | 尚未申请，不填写 | 0x0000000000681830 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不可以。delete[] 已经将旧数组的内存释放，虽然 Memory View 可能仍显示残留字节，但该内存已经归还系统，程序不再拥有访问权限，继续访问属于未定义行为，可能导致程序崩溃或数据错误。
...
> 扩容时必须先复制再释放旧数组，因为如果先释放旧数组，旧数组中的数据就会丢失，无法再复制到新数组中。`size` 表示有效元素的个数，`capacity` 表示数组的总容量；扩容只是增加了可用空间，并没有增加有效元素，所以 `size` 不能设成 `capacity`，否则会把未初始化的内存当作有效元素来处理。