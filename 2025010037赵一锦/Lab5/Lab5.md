...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | （请在CLion Memory View中查看并填写实际地址） | （请在CLion Memory View中查看并填写实际地址） | （请在CLion Memory View中查看并填写实际地址） |
| `newData` 地址 | 尚未申请，不填写 | （请在CLion Memory View中查看并填写实际地址） | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不可以。因为 `delete[]` 已经释放了旧数组的内存空间，该内存区域可能已经被其他程序或系统重新分配使用。如果继续访问旧地址，会导致未定义行为，可能读取到垃圾数据或导致程序崩溃。程序中的 `data` 指针在 `expand()` 中已经被更新为指向新数组，所以必须通过新的 `data` 指针访问元素。
...
> 扩容时必须先复制再释放旧数组，因为如果先释放旧数组，旧数据就会丢失，无法复制到新数组中。`size` 记录的是有效元素的个数，`capacity` 记录的是数组的总容量，两者含义不同。扩容只是增加了数组的存储空间，并没有增加有效元素，所以 `size` 不能直接设为 `capacity`，否则会把未初始化的内存当作有效元素来处理。
