...
> 在此填写你的回答（不少于 20 字）。
p 结点的 16 个字节没有任何变化，因为它是在图 A 之前就已经创建并赋值好的。prev 结点的 16 个字节中，第 5 到第 12 个字节发生了变化。在图 A 中这部分是 prev->next 原本指向的下一个结点（0xc0151b00...），到了图 B，这部分被修改成了新结点 p 的地址（0xd0151b00...）。

2. 图 C 里 `p` 的 `next` 还指着 25 所在的结点，为什么这不会导致 25 被重复释放？如果把 `listRemove` 里的 `free(p);` 删掉，程序输出会不会变？会少掉什么？

> 在此填写你的回答（不少于 30 字）。
因为 C 语言里的指针只是记录了内存地址，它本身并不持有这块内存的“所有权”。执行 free(p) 时，程序只会释放 p 指向的那块内存（也就是存有 data 和 next 的结构体）。p->next 指向 25 所在结点，但这并不会导致 25 被自动释放。listRemove 的逻辑是先通过 p->next 拿到 25 所在结点的地址并保存到局部变量里，然后修改前驱节点的指针跳过这个 25，最后才 free(p) 释放这个已经和链表断开的节点。如果删掉 free(p);，程序在控制台（控制台）打印的输出结果不会变，但被删除的节点所占用的内存将无法归还给系统，从而导致严重的“内存泄漏”。
#### 3.5.5 截图检查标准
...
| 表尾追加复杂度 | `O(n)` | （O（n） |
| 从最后一个结点再走一步到哪里 | 无处可走 | （回到 head (哨兵)） |
...
> 在此填写你的回答（不少于 60 字）。
向量（Vector）的 pushBack 快在均摊 O(1)，因为通常只需在末尾赋值，仅当容量不足时才需 O(n) 扩容；但其在位置 0 插入慢（O(n)），因为需要移动所有后续元素。链表（List）的 pushBack 慢（O(n)），因为每次都要遍历找尾；但其在位置 0 插入快（O(1)），因为只需修改头指针，无需移动元素。
选择场合： 当程序需要频繁在表头或中间插入/删除元素，且对随机访问要求不高时，应选择链表。
