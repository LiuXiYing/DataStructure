...
int circPushBack(CircList* list, int value) {
      /* TODO：insertAfter(circLast(list), value)；size 加一 */
...
void circPrintAround(const CircList* list, int steps) {
    ...
    if (p == list->head) { printf("(empty)"); return; }
    for (int i = 0; i < steps; i++) {
        printf("%d ", p->data);
        p = p->next;
        if (p == list->head) p = p->next;  // 跳过哨兵
    }
    printf("\n");
}
...
void circJosephus(CircList* list, int k) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < k - 1; j++) {
            prev = prev->next;
            if (prev == list->head) prev = prev->next;
        }
        if (prev->next == list->head) prev = prev->next;
        Node* out = unlinkAfter(list, prev);
        printf("%d ", out->data);
        free(out);
        list->size--;
        // 如果摘完后 prev 的下一个是哨兵，让 prev 回到 head
        if (prev->next == list->head) prev = list->head;
    }
    printf("\n");
}
int main(void) {
