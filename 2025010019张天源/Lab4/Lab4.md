...
| C（Lab3 / 课堂） | C++（本次作业） |
| :--- | :--- |
| `SeqList list; list.size = 0;` | IntVector list; |
| `insertAt(&list, 2, 25)` | list.insertAt(2,25); |
| `removeAt(&list, 1, &removed)` | list.removeAt(1,removed); |
| 函数体内的 `list->data[i]` | data[i]; |
| 返回 `int` 的 `1` / `0` | return true;/return false;|
