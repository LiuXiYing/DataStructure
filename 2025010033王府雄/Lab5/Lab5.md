...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | （请从 CLion Memory View 填写实际地址） | （请从 CLion Memory View 填写实际地址） | （请从 CLion Memory View 填写实际地址） |
| `newData` 地址 | 尚未申请，不填写 | （请从 CLion Memory View 填写实际地址） | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不能继续访问旧数组。delete[] 执行后，操作系统已经收回该内存区域的所有权，程序再访问就是未定义行为，可能导致程序崩溃或读取到无效数据。
...
> 先复制再释放旧数组是因为如果先释放旧数组，原数据就丢失了，无法再复制。扩容只改变 capacity（已分配空间大小），不改变 size（有效元素个数），所以扩容后 size 不能设成 capacity，应保持不变。
