...
| :--- | :--- |:---------------| :--- |
| `p` 的地址 | 0xb41750 | 0xb41750       | 0xb41650 |
| `p->data` | 25 | 25             | -1 |
| `p->next` 的地址 | 0xb41670 | 0xb41670       | 0xb41750 |
| `prev->next` 的地址（图 C 填 `list->head->next`） | 0xb41670 | 0xb41750       | 0xb41660 |
| `list->size` | 5 | 5              | 6 |
...
> `p` 这个结点的 16 个字节完全没有变化：它的 `data` 仍是 25（`19 00 00 00`），`next` 仍指向 42 所在的结点，因为插入的两句里 `p` 只被"前驱接住"，它自身没有被修改。`prev` 这个结点（存 -1 的结点）的 16 个字节里：前 4 个字节的 `data` 仍是 `ff ff ff ff` 不变，只有后 8 个字节的 `next` 从"42 所在结点的地址"变成了"`p` 的地址"。

2. 图 C 里 `p` 的 `next` 还指着 25 所在的结点，为什么这不会导致 25 被重复释放？如果把 `listRemove` 里的 `free(p);` 删掉，程序输出会不会变？会少掉什么？

> 因为 `p`（-1 所在的结点）此时已被 `unlinkAfter` 从链上摘下，链上没有任何结点再指向它；`free(p)` 只释放 `p` 自己这 16 个字节，`p->next` 只是 `p` 内部保存的一个地址值，`free` 不会顺着它去释放 25 所在的结点，25 仍由链表持有，只会在 `listDestroy` 时被释放一次，因此不会被重复释放。如果把 `free(p);` 删掉，程序输出不会变（`listDestroy` 沿 `next` 走，`p` 已不在链上，打印的结点数仍是 7），但会少掉一次对 -1 结点堆内存的归还，即 -1 这块 16 字节的堆内存再也不会被释放，造成内存泄漏。
...
| `unlinkAfter` 里"后面没有结点"的判断 | `p == NULL` | `p == list->head`（返回 `NULL`） |
| 表尾追加复杂度 | `O(n)` | `O(n)` |
| 从最后一个结点再走一步到哪里 | 无处可走 | 回到 `head` |
...
> 链表：`pushBack` 慢在必须先沿 `next` 从头走到表尾（要找前驱），这一步 `O(n)`，接链本身 `O(1)`；而 `listInsert(list, 0, v)` 快在哨兵就是现成的前驱，只管一句改指针，不需要移动任何数据。向量：`pushBack` 快在大多数时候只写一个位置并 `size++`，偶尔扩容的 `O(n)` 复制全部均摊后约为 `O(1)`；而在下标 0 处插入要把全部元素整体右移，是 `O(n)`。链表适合：频繁在表头或已知位置插入/删除、规模难以预估、数据量大，但不要求按秩随机访问的场景，例如约瑟夫问题这种"围成圈不断摘人"的需求，天然用循环链表最合适。
