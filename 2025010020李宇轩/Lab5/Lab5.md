...
> **作业目标**：在 Lab4 的 `IntVector` 类基础上，把固定数组改为动态数组；区分 `size` 与 `capacity`；使用构造函数申请空间、析构函数释放空间；实现容量不足时扩大为原来两倍的 `expand()`，让原有接口继续工作。
>
...
> **跳过**：本部分需要 CLion 调试器截图验证，无法在此完成。请自行按照 3.6.1 ~ 3.6.5 的步骤在 CLion 中完成截图和观察记录。
...
> 扩容时先复制再释放旧数组，是因为释放旧数组后其内存中的数据就会丢失，如果先释放再复制就找不到原来的数据了；同时如果先用新地址覆盖 `data`，就会丢失旧数组的地址导致内存泄漏。扩容后不能直接把 `size` 设成 `capacity`，因为扩容只是增加了存储空间大小，并没有插入新元素，有效元素的个数应该保持不变，只有 `insert` 操作完成写入新元素后 `size` 才加一。
