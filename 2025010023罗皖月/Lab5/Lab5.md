...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） |0x00000000001c17f4| 0x00000000001c17ff | 0x00000000001c181f |
| `newData` 地址 | 尚未申请，不填写 | 0x00000000001c181f | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不可以。`delete[]` 已经把这块内存交还给堆管理器，旧数组地址成为无效地址，继续通过它读写是未定义行为：可能读到尚未被覆盖的残留字节、访问到别的对象的内存甚至直接导致程序崩溃。
...
> 扩容时必须先申请新数组并把有效元素复制过去，再释放旧数组，因为旧数组里还存着全部有效元素，一旦先释放，旧地址就不能再读取，元素会全部丢失。扩容只负责给“将来的插入”腾出空间，`size` 记录的是当前有效元素的个数；如果把 `size` 直接设成 `capacity`，就凭空多出一批没有被写入过的无效元素，遍历和查找都会出错。
