...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0x000000001001920 | 0x000000001001920 | 0x000000001001960 |
| `newData` 地址 | 尚未申请，不填写 | 0x000000001001960 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不可以。delete[]执行后旧数组的内存已被释放，虽然调试器可能仍显示残留字节，但该内存区域已归还系统，程序继续访问属于未定义行为可能导致崩溃或数据错误。
...
> 先复制再释放旧数组是因为如果先释放旧数组就丢失了数据无法复制到新数组中。扩容后不能把 size 设成 capacity 是因为 size 表示有效元素个数扩容只是增加了可用空间并没有加入新元素。
