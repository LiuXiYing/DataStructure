...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | `0x55d960031920` | `0x55d960031920` | `0x55d960032160` |
| `newData` 地址 | 尚未申请，不填写 | `0x55d960032160` | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
- [ ] D. 离开 `v` 所在作用域时自动执行析构函数，由本例函数体释放当前数组
...
- [ ] A. 禁止创建两个独立对象
- [ ] B. 避免直接复制指针后两个对象共用数组，造成失效访问或重复释放
- [ ] C. 让 `new[]` 自动变成 `delete[]`
- [ ] D. 禁止在扩容时复制整数元素
...
- [ ] C. 发生扩容的单次追加为 `O(n)`；从空表连续追加时，每次均摊为 `O(1)`
...
- [ ] A. `O(1)`，因为可以自动扩容
- [ ] B. `O(n)`，仍可能移动大量元素；扩容复制与右移两段线性工作相加仍为线性
- [ ] C. `O(log n)`，因为容量每次乘二
- [ ] D. `O(n²)`，因为存在扩容循环和插入循环
...
| :--- | :---: | :---: | :---: |
| `IntVector v;` 创建后 | （填写） | （填写） | （填写） |
| `v` 追加 `40` 后 | （填写） | （填写） | （填写） |
| 满表 `v.insert(5, 99)` 失败后 | （填写） | （填写） | （填写） |
| `v` 追加 `50` 后 | （填写） | （填写） | （填写） |
| 满表 `v.insert(4, 28)` 后 | （填写） | （填写） | （填写） |
| 六次删除完成、`v` 删除全部元素后 | （填写） | （填写） | （填写） |
