...
/* 返回秩为 rank 的结点的前驱；rank == 0 时返回哨兵。0 <= rank <= size */
...
/* 练习2：空表判断。不许用 size，用指针关系判断 */
int circEmpty(const CircList* list) {
    return list->head->next == list->head;
}

/* 练习3：遍历。从 head->next 出发，回到 head 就停；写成 p != NULL 会死循环 */
...
/* 练习4：追加到表尾。在 circLast 后面插入，size 加一，返回 1 */
...
/* 练习5：按秩插入，合法范围 0 <= rank <= size。与 listInsert 一字不差 */
...
/* 练习6：按秩删除，合法范围 0 <= rank < size。
 * 与 listRemove 只差 unlinkAfter 多传一个 list */
...
/* 练习7：按值查找，返回第一次出现的秩，找不到返回 -1。停止条件是 head，不是 NULL */
...
/* 练习8：从第一个结点出发连续走 steps 步，每到一个结点输出它的 data；
 * 走到哨兵就跳过哨兵。走的步数可以超过 size，用来证明这是一个圈。空表时输出 (empty) */
...
/* 练习9：约瑟夫问题。从第一个结点开始报 1，报到 k 的结点出列（摘下、输出、释放），
...
void circJosephus(CircList* list, int k) {
    ...
    while (list->size > 0) {
        ...
        free(out);
        list->size--;
        /* 如果 prev 后面是哨兵，把 prev 拨回哨兵（下一轮从第一个有效结点报 1） */
