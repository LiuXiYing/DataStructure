...
add\_executable(lab6\_list linkedlist.c)

add\_executable(lab6\_circ circlist.c)
...
\- 有且只有两个 `add\_executable`，目标名与源文件名与上面完全一致。
...
1\. CMake 配置选择 \*\*Debug\*\*，运行目标选择 `lab6\_list`，重新构建后用 \*\*Debug\*\* 启动。
...
\*\*A．新结点已接住后面、前驱还没改：`imgs/insert\_before\_link.png`\*\*
...
!\[图 A：新结点已接住后面，前驱还没改](imgs/insert\_before\_link.png)



\*\*B．前驱改完之后：`imgs/insert\_after\_link.png`\*\*
...
!\[图 B：前驱改完之后](imgs/insert\_after\_link.png)



\*\*C．已断链、尚未释放：`imgs/remove\_before\_free.png`\*\*
...
!\[图 C：已断链、尚未释放](imgs/remove\_before\_free.png)
...
| :--- | :--- | :--- | :--- |
| `p` 的地址 | 0x1e1840 | 0x1e1840 |0x1e1740 |
| `p->data` | 25 |25 | -1 |
| `p->next` 的地址 | 0x1e1780  | 0x1e1780 | 0x1e1840|
| `prev->next` 的地址（图 C 填 `list->head->next`） | 0x1e1780  | 0x1e1840  |0x1e1700|
| `list->size` | 5 |6| 5 |
...
图 A 到图 B 之间，p 这个结点的 16 个字节没有变化：它的 data（25）和 next（指向 42 所在结点的地址 0x1e1780）都保持不变。prev 这个结点的 16 字节里只有后 8 字节的 next 指针变了，从前指向 42 所在结点（0x1e1780）变成了指向 p（0x1e1840）；前 4 字节 data（-1）和中间的填充字节都没有变。
...
图 C 里 p 的 next 仍指向 25 所在的结点（0x1e1840），但这不会导致 25 被重复释放，因为 p 已经被 unlinkAfter 从链上摘下来了，list->head->next 已经绕过 p 直接指向 18 所在结点，之后所有遍历和释放都从 head 出发，不会再经过 p。p 本身只被 listRemove 里的 free(p); 释放一次。如果把 free(p); 删掉，程序输出基本不变，但会少释放 -1 所在的这个结点，造成内存泄漏，listDestroy 打印的结点数会少 1。
...
| 遍历停止条件 | `p != NULL` | p != list->head |

| 最后一个结点的特征 | `p->next == NULL` |  p->next == list->head |

| `unlinkAfter` 里"后面没有结点"的判断 | `p == NULL` | p == list->head |

| 表尾追加复杂度 | `O(n)` | O(n)（无 tail 时） |

| 从最后一个结点再走一步到哪里 | 无处可走 | 回到哨兵 head |
...
listPushBack 是 O(n)，因为单链表只有头哨兵，没有尾指针，每次追加都要从哨兵走到最后一个结点；而 IntVector::pushBack 是均摊 O(1)，因为向量在尾部有连续空间，容量够时直接在末尾写入。listInsert(list, 0, v) 是 O(1)，因为表头插入只需改哨兵和新结点的两根指针；而 IntVector 在下标 0 处插入是 O(n)，因为需要把后面所有元素整体后移。两者"快"的地方正好相反：链表快在"已知前驱时的插入/删除"，向量快在"随机访问和尾部追加"。因此，当操作集中在表头/表尾附近、或需要频繁在已知位置插入删除、且不需要随机访问时，应选链表；当需要频繁按下标访问、主要在尾部增删时，应选向量。
...
&#x20;       ├── insert\_before\_link.png

&#x20;       ├── insert\_after\_link.png

&#x20;       └── remove\_before\_free.png
...
\- `CMakeLists.txt` 必须包含 `lab6\_list`、`lab6\_circ` 两个目标，源文件分别为 `linkedlist.c`、`circlist.c`。
